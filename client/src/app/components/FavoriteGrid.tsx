...
const FavoriteGrid = () => {
  ...
  const { products, fetchLoading } = useProductsContext();
  ...
  return (
    <section className="pt-7.5 md:pt-15 pb-7.5 md:pb-15" id="souvenirs">
      ...
      sm:text-[23.05px] mb-7.5"
      >
        Tourists Favorites
      </h2>
      {fetchLoading && (
        <div className="w-full flex justify-center">
          <Loader2 className="animate-spin mt-4" color="black" size={40} />
        </div>
      )}
      <div className="relative max-w-[90vw] mx-auto">
        <div
          className="font-grotesk-400 flex overflow-x-auto hide-scrollbar gap-2.5 sm:gap-5 "
          ref={containerRef}
        ...
        >
          {productsToRender.map((product, index) => (
            <motion.div
              ...
              className="bg-[#FFF6F1] shadow-xs cursor-pointer group overflow-hidden flex-shrink-0 w-[202px] lg:w-[315px]"
            >
              <Image
                src={product.image}
                alt={product.name}
                height={330}
                width={330}
                className="transition-transform duration-300 ease-in-out group-hover:scale-105 object-cover w-full h-auto"
              />
        ...
        <div className="flex justify-end mt-7.5 md:mt-11 gap-3.5 right-0 top-full">
          <Image
